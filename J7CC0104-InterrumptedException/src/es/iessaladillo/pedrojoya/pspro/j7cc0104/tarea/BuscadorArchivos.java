...
public class BuscadorArchivos implements Runnable {
	...
	public void run() {
		...
		if (file.isDirectory()) {
			try {
				...
				System.out.printf("%s: Se ha interrumpido el hilo de búsqueda", Thread.currentThread().getName());
			}
		}
	}
	 ...
	 * @param carpeta: Directorio en el que buscar
	 * @throws InterruptedException: Si el hilo es marcado como interrumpido.
	 */
	...
	/**
	 * Procesa un archivo comparando su nombre con el nombre del.
	 * archivo buscando. Si es el buscado muestra su path absoluto.
	 * @param archivo: Archivo a procesar.
	 * @throws InterruptedException: Lanza esta excepción si el hilo
	 * está marcado como interrumpido.
	 */
	private void procesarArchivo(File archivo) throws InterruptedException {
		...
		if (archivo.getName().equals(nombreArchivo)) {
			...
			System.out.printf("%s : %s\n", Thread.currentThread().getName(), archivo.getAbsolutePath());
		}
		// Si he sido marcado como interrumpido genero la excepción
		// correspondiente que se propagará hacia el método llamador.
		if (Thread.interrupted()) {
