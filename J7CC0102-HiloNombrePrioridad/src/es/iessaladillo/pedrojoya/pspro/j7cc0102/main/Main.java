...
public class Main {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// de cada hilo antes de iniciarse (NEW), imprimiéndolo en el archivo.
			escritor.println("*******************************************************************");
			for (int i = 0; i < 10; i++) {
				escritor.println(hilos[i].getName() +  " - Estado: " + hilos[i].getState());
			...
			while (!finalizados) {
				...
				finalizados = true;
				for (int i = 0; i < 10; i++) {
					finalizados = finalizados && (hilos[i].getState() == State.TERMINATED);
				}
			}
	 ...
	 * @param escritor : Escritor para escribir los datos.
	 * @param hilo : Hilo cuya información quiere escribirse
	 * @param estadoAnterior : Estado anterior del hilo.
	 */
	private static void escribirInfoHilo(PrintWriter escritor, Thread hilo, State estadoAnterior) {
		escritor.println("*******************************************************************");
		escritor.println(hilo.getName() + " - Id: " + hilo.getId());
		escritor.println(hilo.getName() + " - Prioridad: " + hilo.getPriority());
		escritor.println(hilo.getName() + " - Estado anterior: " + estadoAnterior);
		escritor.println(hilo.getName() + " - Estado actual: " + hilo.getState());
		escritor.println("*******************************************************************");
	}
